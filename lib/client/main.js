...
async function loadPageState() {
  ...
  if (section === 'collections') {
    if (itemName) {
      if (itemId) {
        await loadAndRenderItem(itemName, itemId)
      } else {
        await renderCollection(itemName)
      }
    }
  }
}
...
dom.collections.innerHTML = /*html*/`
  <ul>
    ${collectionNames.map(collectionName => /*html*/`
      <li><a href="#/collections/${collectionName}" data-type="collection" data-collection="${collectionName}">${collectionName}</a></li>
    `).join('\n')}
  </ul>
`
...
async function renderCollection(collectionName) {
  if (!dom.main) return
  /** @type {CollectionItem[]} */
  const items = await fetch(`api/collections/${collectionName}`).then(o => o.json())
  const html = /*html*/`
    <div class="item-list">
      ${items.map(item => /*html*/`
  ...
  ensureTag('div', dom.main.querySelector('.item-list')).addEventListener('click', async evt => {
    ...
    if (!id) return
    await loadAndRenderItem(collectionName, id)
  })
}


/** @type {(collectionName: string, id: string) => Promise<void>} */
async function loadAndRenderItem(collectionName, id) {
  /** @type {CollectionItem} */
  const item = await fetch(`api/collections/${collectionName}/${id}`).then(o => o.json())
