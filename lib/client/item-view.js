...
import { createFromHTML, isTag, ensureTag } from './dom-utils.js'

const dom = {
  languageStyles: ensureTag('style', document.querySelector('#language-styles')),
}
...
async function saveChanges(collectionName, item) {
  ...
  const changes = []
  for (const element of ensureTag('form', document.querySelector('form')).elements) {
    if (!isTag('input', element) && !isTag('textarea' ,element)) continue
  ...
  try {
    const newItem = await jsonFetch(`api/collections/${collectionName}/${item.meta.id}`, {
...
function setLanguageStyles(lang) {
  dom.languageStyles.textContent = `.property-viewer [lang]:not([lang="${lang}"]) { display: none; }`
}