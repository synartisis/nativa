...
export async function router(req, res) {
  ...
  if (url.pathname.startsWith('/collection/') && req.method === 'GET') {
    ...
    if (!_id) {
      ...
      const item = await rawdb.getItem(collectionName, _id, true)
  ...
  if (url.pathname.startsWith('/collection/') && req.method === 'POST') {
    const [ , base, collectionName, _id ] = url.pathname.split('/')
    if (!_id) { res.statusCode = 404; res.end(); return }
    const body = await readRequestBody(req)
    const changes = JSON.parse(body.toString())
    try {
      await rawdb.applyChanges(collectionName, _id, changes)
      const modifiedItem = await rawdb.getItem(collectionName, _id, true)
      res.setHeader('content-type', 'application/json')
