...
export async function router(req, res) {
  ...
  if (req.session) setState({ username: req.session.username, role: req.session.roles })
  ...
  if (url.pathname === '/auth/login' && req.method === 'POST') {
    ...
    switch (checkPasswordResult) {
      case 'ok':
        await sessions.createSession(req, res, username)
        return redirect(res, '../')
      case 'wrong':
        res.statusCode = 401
  ...
  if (url.pathname === '/auth/logout' && req.method === 'GET') {
    sessions.removeSession(req, res)
    return redirect(res, '.')
  }
  if (env === 'development') {
  ...
  if (url.pathname === '/api/user/register' && req.method === 'POST' && req.session?.roles.includes('admin')) {
    const body = await readRequestBody(req)
  ...
  if (url.pathname === '/api/user/password-reset' && req.method === 'POST' && req.session?.roles.push('admin')) {
    const body = await readRequestBody(req)
    const { username } = JSON.parse(body.toString())
    if (!username) { res.writeHead(400, 'username is required'); res.end(); return }
