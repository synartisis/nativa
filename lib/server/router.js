...
export async function router(req, res) {
  ...
  if (req.session) setState({ username: req.session.username, role: req.session.role })
  ...
  if (url.pathname === '/api/user/register' && req.method === 'POST' && req.session.role === 'admin') {
    const body = await readRequestBody(req)
  ...
  if (url.pathname === '/api/user/password-reset' && req.method === 'POST' && req.session.role === 'admin') {
    const body = await readRequestBody(req)
    const { username } = JSON.parse(body.toString())
    if (!username) { res.writeHead(400, 'username is required'); res.end(); return }
